...
def conv():
    _input = Input(shape=(PAMAP.dct_length*3*len(PAMAP.sensors), 1))
...
for test_id in test_ids:
    ...
    _train_labels_ = np_utils.to_categorical(_train_labels, len(PAMAP.activity_list))
    _test_labels_ = np_utils.to_categorical(_test_labels, len(PAMAP.activity_list))
    ...
    _input_ = Input(shape=(PAMAP.dct_length*3*len(PAMAP.sensors), 1))
    base_network = conv()
    ...
    PAMAP.write_data('conv.csv', 'score'+','.join([str(f) for f in results])+'knn_acc'+str(acc))
